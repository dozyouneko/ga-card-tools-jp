...
- それ以外（カードDB / ひとくちキーワード解説） … 公式API `https://api.gatcg.com` への `connect` / `img`、
  および Cloudflare Web Analytics への `script`（`static.cloudflareinsights.com`）と
  計測データ送信の `connect`（`cloudflareinsights.com`）のみ追加許可
...
- [x] 翻訳精度の見直し（用語辞書の表記統一・不自然な言い回しの修正を全セットで実施）
- [ ] カードDBの絞り込み強化（サブタイプ検索・ハイライトは実装済み。コスト・レベル等のフィルタは未着手）
- [x] リポジトリ整理（ルート直下の `scratch_ref_*` 作業用ファイルを削除。翻訳作業完了に伴い不要と判断）
