...
├─ tournaments/            大会デッキページ（静的生成。一覧 / 大会詳細＋順位表 / デッキ断片 decks.html）
├─ data/
│  ├─ translations.js      日本語訳データ（用語辞書・メタ訳・カード個別訳の基盤）
│  ├─ tl/                  セット別の翻訳ファイル（全セット分）
│  └─ tournaments/         大会データ（Omnidex APIから取得。index.json / events/ / pending.json）
│
├─ tools/
...
- 公式Omnidex API（`api.gatcg.com/omnidex/events/*`）から**主要大会**（Store Championship・Regionals・
  Nationals・Ascent）の順位表・提出デッキを取得し、**日本語訳付き**の静的ページとして生成
- 大会一覧（カテゴリ・国・フォーマット・シーズン・期間・大会名/プレイヤー名/大会IDで絞り込み）→ 順位表 →
  「デッキを見る」でデッキをダイアログ表示（本体は `tournaments/<id>/decks.html` から取得）
- デッキはカード画像タイル表示。カードをクリックするとカードDBと同じ日本語の詳細ダイアログが開く
- データ更新は [GitHub Actions](.github/workflows/build-tournaments.yml) が日次で自動実行（下記「公開方法」参照）
...
`build:tournaments` は公式Omnidex APIのイベントidを順に走査して新しい大会を探します
（一覧・検索APIが存在しないため）。取得済みidは `data/tournaments/index.json` に記録され、
2回目以降は新規idと保留中の大会だけを見ます。`--no-scan` を付けるとAPIを叩かず既存データから
ページだけを再生成します。**sitemapへの反映は `build:tournaments` → `build:cards` の順**に実行してください。
...
[.github/workflows/build-tournaments.yml](.github/workflows/build-tournaments.yml) が
**毎日 03:00 JST** に大会スキャン → ページ生成 → 差分があれば `main` へ直接 push します
（push をトリガーに Cloudflare Pages の自動デプロイが動きます）。

- 動作確認・障害時は GitHub の Actions 画面から `workflow_dispatch` で手動実行できます
- コミット対象は `data/tournaments` / `tournaments` / `sitemap.xml` のみ（カードページは含めません）
