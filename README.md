...
├─ tournaments/            大会入賞デッキページ（静的生成。一覧 / 大会詳細 / デッキ詳細）
├─ data/
...
### 大会入賞デッキ検索（`/tournaments/`）

- 公式Omnidex API（`api.gatcg.com/omnidex/events/*`）から**主要大会**（Store Championship・Regionals）の
  順位表・提出デッキを取得し、**日本語訳付き**の静的ページとして生成
- 大会一覧（カテゴリ・国・フォーマット・大会名/プレイヤー名で絞り込み）→ 順位表 → デッキ詳細
- デッキ詳細はカード画像タイル表示。カードをクリックするとカードDBと同じ日本語の詳細ダイアログが開く
