...
// ⚠ 報告される11件（2026-08-30 実測）は「用語解説には出るが訳文に中核語が無い」型で、
//   実在するが無害（解説が1行余分に出るだけ）。ここで exit code を変えて赤くすると
//   使われなくなるため、報告のみに留める（#40 の教訓）。
// ⚠ 逆向きの欠落（訳文に用語があるのにハイライトされない）は、このツールでは
//   原理的に検出できない（英語ゲートに当たったものしか対象にしないため）。
//   そちらは npm run validate（生成済みカードページを走査する検査）が exit 1 で止める。
