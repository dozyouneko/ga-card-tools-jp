...
@media (max-width:640px) {
  /* #21: ヘッダの縦占有を減らす。ブランドは文字を落としてマークだけ残す(リンクは生きる)。
     ⚠ font-size:0 は「テキストだけ消す」ための手段。display:none にするとトップへ戻れなくなる。
     ⚠ flex-wrap:nowrap だけでは 92→102px と悪化する。文字を落とすところまでやって初めて効く。
     ⚠ 同じ内容が scripts/build-card-pages.mjs の CARDS_CSS にもある(片方だけ直すと高さが食い違う) */
