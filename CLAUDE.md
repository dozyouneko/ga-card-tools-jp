...
- `npm run build:tournaments` — 大会デッキ(#14)のスキャン+ページ生成。
  ...
  - 順位表の属性玉(#15)は `scripts/lib/element-orbs.json`(コミット済み)を読むだけ。
    **`node scripts/gen-element-orbs.mjs` は手動実行専用**(公式カード画像を取得して32px WebPに
    切り出す。sharp が必要)。新しい属性が増えたときだけ再生成し、出力JSONをコミットする。
    日次cronのビルドから呼んではいけない(ネットワークと画像処理ライブラリに依存するため)
