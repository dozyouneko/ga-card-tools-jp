...
| 日付 | 版 | 内容 |
|---|---|---|
| 2026-07-27 | **完了** | ユーザーの公開判断を得て push(`73f8940b..ae9d474b`)。**検証8〜10を全項目クリアし #30・#29 をクローズ**。⭐ 決定的な証拠は **cronの実運用コミット `931b94d3`(2026-07-27 11:31 UTC)が `data/tournaments/index.json` の2行だけだったこと** — CIはクリーンチェックアウトから**APIを取り直して**カード2,240ページ・セット56ページ・`sitemap.xml`・索引・`featured-sets.json` を再生成しており、それが**すべてコミット済みとバイト一致**した。ローカル2回実行より強い決定性の実証で、「新セットが無い日はノイズコミットが出ない」という案A採用の前提が本番で確定した。設計担当も独立に本番実測: 本番 `sitemap.xml`(200 / 522,082バイト / 2,743URL)と `origin/main` の実ページを突合し**カード2240/2240・セット56/56 で乖離0・孤児0**、`/data/card-meta-index.json`・`/data/featured-sets.json` は**リポジトリ実物と `cmp` でバイト一致**、無作為抽出8件のカードURLはすべて200。索引の個別取得は22件を約9秒・失敗0で完走。**R3(孤児ページ)も本番で0件**。変更4の `CLAUDE.md` 更新は設計担当が実施(push待ち) |
