...
if (jpCand === null) {
  ...
  jpSortDropped = s.unknown === base.length && sortField() !== "name";
}
```
...
| V15 | 〃 | コスト順 ▼ | 全 **208** 件。先頭 `青龍、蒼き龍(10) → 伝説の瑠璃運命石(10) → 朱雀、朱き不死鳥(7) → トリスタン、影を裂く者(3) → 涙滴の宝冠(3)`。⚠️ **`青龍、蒼き龍` はフリップ面のslugで、索引には表面（`fabled-azurite-fatestone`）の値が入る**ため `伝説の瑠璃運命石` と同点10で並ぶ（実行時の `fetchCard()` が表面を返す挙動と一致） |
