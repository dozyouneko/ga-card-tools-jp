...
| 版 | 日付 | 内容 |
|---|---|---|
| 第4版 | 2026-08-11 | ⚠️ **本設計書の前提が2つ誤っていたことが実測で判明したので訂正**（詳細は `docs/design/62-cronの赤の解消/cronの赤の解消_設計.md` §2-2）。①**§2.3 の「403 = key not valid（キーファイルが無い・中身が違う）」は誤り** — 誤ったキーで送ると **202** が返る。403 は**サイトの認可**の問題で、キーの誤りの信号ではない ②**§5 リスク表の「キーファイルを消してしまう → 403 → jobが赤くなって気づける」は成り立たない**（202が返るので誰も気づけない）③**§4 の V10（誤ったキーで403 → exit 1）は再現不能**。⭐ 併せて、**導入以来IndexNowは一度も通知に成功していない**ことが判明（緑だった日は通知対象0件でAPIを呼んでいない日）。cronの信号を回復させる暫定対処は **#62** に切り出した。**#38 自体は `保留` のまま**（本質的な解決は #28 の判断待ち） |
...
| **403** | ⚠️ **第4版で訂正**: key not valid **ではない**。**サイトの認可**の失敗（`UserForbiddedToAccessSite`） | ❌ **exit 1**。⚠️ **ただし #62 で例外を1つ追加**（下記） |
| **422** | URLがhostに属さない（`InvalidRequestParameters`） | ❌ **exit 1** |
...
#### ⚠️ 第4版の訂正（2026-08-11 実測）— 上の表の403の行は誤りだった

| 送ったもの | 実測 |
|---|---|
| **正しいキー**（本番の現行設定） | **403 `UserForbiddedToAccessSite`** |
| **誤ったキー**（そのキーファイルは存在しない） | **202**（403ではない） |
| URLが `host` に属さない | 422 `InvalidRequestParameters` |

- ⚠️ **403 は「キーの誤り」の信号ではない。** よって
  **§5 リスク表の「キーファイルを消してしまう → 403 → jobが赤くなって気づける」は成り立たない**
  （消しても202が返るので誰も気づけない）。**V10 も再現不能**
- ⭐ **キーファイルの健全性は IndexNow の応答では検出できない。**
  自前で検査する仕様を **#62 §5-2** に置いた
- ⚠️ **#62 で403に例外が1つ入る**: `errorCode` が `UserForbiddedToAccessSite` **かつ**
  `host` が `*.pages.dev` のときだけ**警告に留めて exit 0**。
  それ以外の403は**この表のとおり exit 1 のまま**。
  ⭐ 独自ドメインへ移ると `host` が変わり、**抑制は自動で解除される**
...
| **V10** | 誤ったキーで送る（キーを1文字変える） | ⚠️ **第4版で再現不能と判明**（実測は **202**）。この項目は**破棄**し、代わりに **#62 §5-2 の自前検査（検証B-2）**で「誤ったキーなら送信前に exit 1」を担保する |
...
| リスク | 対処 |
|---|---|
| キーファイルを消してしまう | ⚠️ **第4版で訂正: この対処は成り立たない。** 消しても IndexNow は **202** を返すため**誰も気づけない**（§2.3の訂正）。⭐ 自前の検査を **#62 §5-2** で足す |
