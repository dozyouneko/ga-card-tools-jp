...
| 2026-08-19 | 第9版 | ⭐ **#73 完了 — 未訳254枚が0枚になった**（翻訳済み総数 2,262 → **2,516枚**）。バッチ3（34枚＋`fulgurite-coordinator` の改名）は `9b472a88` で実装・**一発合格**・承認。PRD系7prefixすべて未訳0、PRD系の全セットページが「訳済み＝総カード行」。Boon 12枚は既訳パターンに**違反0**、`Coordinator` 3枚も「指揮者」で統一済み。⚠️ **§13に書いたフレーバー抽出式は設計側の誤りだった**（空白のみの文字列を truthy で拾う）。⭐ 設計確認中に**別の不具合を発見**: `editions[].flavor` にフレーバーがある**473枚で原文が一切表示されない**（消費側2箇所が `card.flavor` しか見ていない）。⚠️ **#73のスコープ外なので新規issueに切り出す**（`issue反映待ち_2026-08-19.md`）。⚠️ 未pushのまま（GitHubアカウント停止中） |
...
#### 第6版で追加した確定訳（ユニット1のレビュー合格後・設計担当が判断）

サブバッチAの実装で「既訳に前例が0件」だった語。⚠️ **`DisCorp` と `Resonator` は
フレーバー語ではなく実データ上の「サブタイプ」**（`DISCORP` 20枚・`RESONATOR` 9枚）で、
**効果文にも出る**ため、ユニット2に入る前に確定させた。

| English | **確定訳** | 判断の根拠 |
|---|---|---|
| **DisCorp** | **ディスコープ** | §2-6・§2-7 の**ブランド名はカタカナ**（`VelTech → ヴェルテック`）と同じ扱い。⚠️ 効果文2枚（`acheron-express-officer`・`tower-of-dis`）に出る |
| **Resonator** | **レゾネーター** | 同上（サブタイプ名・効果文3枚に出る） |
| **Higher Plane** | **上位界** | ブランドではなく世界観用語のため意訳。フレーバー3枚のみ |
| **Disonian** | **ディソニア人** | フレーバー1枚のみ。`Dis` の住人 |
| **Core Fractal** | **コアフラクタル** | ⭐ **§2-7 の `... Fractal → ◯◯のフラクタル` の例外**。バッチ1が効果文で既に「コアフラクタル・トークン」と書いており、**セット内で先に確定した表記を優先**する（`core-fractal` は §11-B にある） |

⚠️ **`Core Fractal` を「コアのフラクタル」に変えない。** 変えるとバッチ1の効果文と
サブバッチAの `aenean-crystallization` の**計2箇所の修正が必要**になり、得るものが無い。

#### Droid の接続形（第6版で追認）

**名詞修飾は直結・形容詞的修飾は `の`／`な`** で統一する（ユーザー確定訳5枚がこの形のため）。
例: `セル鍛造ドロイド`（名詞）／`頑丈なドロイド`（形容詞）／`霞のドロイド`（名詞＋の）。
...
⭐ **2026-08-19時点: バッチ1・2が完了し、実測値は上表の「バッチ2完了後」と全項目一致**（PRD 0 / 1st 5 / P 12 / SD 10 / DP 7・ユニーク残 34）。
...
4. **バッチ1〜3すべてのpush（本番公開）の可否** — 発売日 8/21 との兼ね合いでユーザーが判断する。
...
## 12. バッチ3（34枚）への申し送り（第7版）

バッチ2で判明した、**バッチ3の実装前に知っておくべきこと**:

1. ⭐ **フレーバーは2箇所を見る**。`card.flavor || (card.editions||[]).map(e=>e.flavor).find(Boolean)`。
   ⚠️ **バッチ2はこれで1往復差し戻しになった**。バッチ3の34枚に該当は**0枚**（原文flavorあり17枚は
   すべて `editions[].flavor` 側）だが、**式は正しい形で書くこと**
2. **命名ルールは §2-7（第6版）がそのまま使える**。確定訳5語もバッチ3に適用する
3. ⚠️ **`data/translations.js` の `subtypes` / `terms` の追加は、この設計書のスコープ外**
   （フォローアップissueで別途行う）。バッチ3でも **`translations.js` の差分は0**を維持する
...
|---|---:|---:|
| PRD系の未訳カード | **254** | **0** |
| 翻訳済みカード総数 | 2,262 | **2,516** |
| PRD系セットページの訳済み率 | 一部 | **7セットすべて 100%** |
| `data/translations.js` への `terms` 追加 | — | **8件（バッチ1のみ）**／バッチ2・3は0 |

### ⚠️ この設計書から漏れた・誤っていたこと（次に活かす）

1. ⚠️ **§13 のフレーバー抽出式が誤りだった** — `card.flavor || …` は**空白のみの文字列を truthy で拾う**。
   正しくは `trim()` してから判定する。⭐ **バッチ2の差し戻し（M1-1）の再発防止として書いた式自体に穴があった**
2. ⚠️ **`Coordinator` の訳がバッチ1と2で割れた** — 語族の追跡が**バッチをまたぐと切れる**。
   §2-7・§2-8 のような命名表は**バッチごとに作るのではなく、セット単位で一度作って持ち回る**べきだった
3. ⚠️ **辞書の抜けを検出する経路が無い** — `subtypes` 5種・`terms` 2種の欠落は
   `npm run validate` を**緑のまま通過**した（#40 と同型）
4. ⭐ **「issueが唯一の引き継ぎ手段」は障害に弱い** — 2026-08-19 のGitHubアカウント停止で
   issueが読み書き不能になり、**`docs/` への保全でしのいだ**。
   記録の物理的な置き場が1つしかないことのリスクが顕在化した

### 残っている作業

- ⚠️ **push（本番公開）** — 未pushコミットが多数。**ユーザー判断＋アカウント復旧待ち**
- **フォローアップissue 2件の起票**（`issue反映待ち_2026-08-19.md` に本文を用意済み）
  1. PRDで増えた辞書項目の追加（`subtypes` 5種・`terms` 2種・`TRANSLATION.md` の `Retort`）と検出経路
  2. `editions[].flavor` の473枚でフレーバー原文が表示されない不具合
- **`TRANSLATION.md` への追記**（§6）— 上記1に統合済み
