...
| 日付 | 版 | 内容 |
|---|---|---|
| 2026-07-26 | **完了** | ユーザーの公開判断を得て push(`5a4a05f9..b1e71cb4`)。**本番検証を全項目クリアし issue #27 をクローズ**。P1・P2 は設計担当が実測(200/`application/json`・生126 KB / gzip転送33 KB・配信物とリポジトリが `diff` で完全一致・デッキ構築の相対パス `../../data/…` が本番で200に解決)、**P3(CSP違反なし)・P4(トップとデッキ構築で検証1〜3の再現)はユーザーが実ブラウザで確認**。デッキ構築はローカルでD1未起動のため画面完走できていなかったが、本番で画面として確定。派生課題は #29(索引の鮮度)・#30(sitemapとカードページの乖離)に切り出し済みで本設計の範囲外 |
| 2026-07-26 | 実装レビュー | 開発担当の実装(`77a24a84`)を設計適合レビューし**承認**。設計担当側で**報告値を使わず独立に検算**: 索引カバレッジ **2262/2262**(欠落0・余剰0。`loadPageI18n` が `index.html` のscript並びを読むためブラウザの `I18N.cards` と完全一致)、スナップショット突合 **2240件中 不一致0**、辞書ID範囲外参照0、サイズ 生126.3 KB / gzip **33.9 KB**(設計見積 125.2/33.5)。`metaMatches()` が `matchesActiveFilters()` と同一規則であることをコードで確認。**開発担当の指摘6を検証し、初版の「誤った結果は出ない」は誤りと判定**(下記「保証の範囲」で訂正。取りこぼし3ケースを実測で false と確認)。確認事項4(デッキ構築の件数据え置き)は**据え置きで承認**(従来は常に絞り込み前の総数=より不正確だったため厳密に改善)。`isApproxTotal` 削除も残参照0で承認。**残: pushはユーザーの公開判断待ち**(本番配信物を含む)+ 本番検証P1〜P4。索引鮮度の恒久対策は **#29** に切り出し |
...
⚠️ **保証の範囲(2026-07-26の実装レビューで初版の記述を訂正)**

初版はここに「索引が古い場合の影響は表示件数が不揃いになるだけで**誤った結果は出ない**」と書いていたが、
**これは誤り**。二段構えが守るのは**過剰包含(over-inclusive)方向のズレだけ**である。

`metaMatches()` が `false` を返した slug は候補に入らず**そもそも取得されない**ため、
`matchesActiveFilters` に届かない。つまり **fail-open が守っているのは「索引に *無い* slug」だけ**で、
**「索引に *あるが内容が古い* slug」は守られず、黙って落ちる**。

実測で確認した取りこぼし(設計担当・2026-07-26):

| ケース | 起きること | 候補に残るか |
|---|---|---|
| **新たに禁止された**カード × 「スタンダード禁止」で絞り込み | 索引の `banned` が空のまま → **出てこない** | ❌ false |
| **禁止解除**されたカード × 「スタンダードで使用可」で絞り込み | 索引に `banned` が残る → **出てこない** | ❌ false |
| **再録**されたカード × 新エキスパンションで絞り込み | 索引に新 prefix が無い → **漏れる** | ❌ false |
| 索引に**無い** slug(新規翻訳・生成漏れ) | 取得後フィルタが判定する | ✅ true(fail-open) |

腐り得るのは **`bannedFormats`(禁止改定)と `setPrefixes`(再録)の2項目だけ**。
`classes` / `elements` / `types` / `subtypes` は刷られたカードで変わらないため陳腐化しない。

→ **正しい保証の言い方**: 「**索引に無い slug は落とさない。ただし索引の内容が古い場合は
`bannedFormats`・`setPrefixes` に関して取りこぼしうる**」。これは案1の構造上の限界であり、
実装の欠陥ではない。**再生成トリガーは「新カードを翻訳したとき」だけでは不足で、
「禁止改定があったとき」「再録が出たとき」も必要**(→ CLAUDE.md に明記。恒久対策は #29)
...
| 項目 | 状態 |
|---|---|
| 変更1 索引生成スクリプト+生成物 | ✅ **実装済み**(`77a24a84`・`scripts/gen-card-meta-index.mjs` / `data/card-meta-index.json`) |
| 変更2 取得前フィルタ+フォールバック | ✅ **実装済み**(`77a24a84`・フォールバック3条件を実機確認) |
| 変更3 件数の正確化+文言 | ✅ **実装済み**(`77a24a84`・`isApproxTotal` は `jpApprox` に置換) |
| 変更4 `jpPageSize` 据え置き | ✅ 変更なし(据え置きを確認) |
| CLAUDE.md への再生成タイミング追記 | ✅ **設計担当が実施**(2026-07-26。**禁止改定・再録も**トリガーに含めた) |
| push | ✅ **完了**(2026-07-26・ユーザーの公開判断を得て `5a4a05f9..b1e71cb4`。デプロイ反映は約20〜40秒) |
| 本番検証 P1・P2 | ✅ **合格**(設計担当が実測。200/`application/json`・生126 KB / gzip転送33 KB・配信物とリポジトリが `diff` で完全一致) |
| 本番検証 P3・P4 | ✅ **合格**(2026-07-26・ユーザー確認。CSP違反なし・トップとデッキ構築で検証1〜3を再現) |
| 索引鮮度の恒久対策(日次cronでの自動再生成) | → **[#29](https://github.com/dozyouneko/ga-card-tools-jp/issues/29) に切り出し**(本issueの範囲外) |
